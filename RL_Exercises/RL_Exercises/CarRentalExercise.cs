...
{
    public class CarRentalExercise
    {
        ...
        {
            // since Jack's employee can take a car from A to B, if action is > 0, then subtract 1 to account for the additional car
            var payableAction = (action > 0) ? action - 1 : action;
            double returns = -1 * Math.Abs(payableAction) * TRANSFER_COST;
            foreach (var rentalCountA in Enumerable.Range(0, 11))
            {
                foreach (var rentalCountB in Enumerable.Range(0, 11))
                {
                    ...
                    reward = carsLeftAtA > 10 ? reward - 4 : reward;
                    reward = carsLeftAtB > 10 ? reward - 4 : reward;
